  ...
  return (
    <div className={`fixed w-full z-100 flex justify-center transition-all duration-500 ${scrolled ? 'pt-4' : 'pt-8'} px-4`}>
      ...
      >
        ...
        <div className="hidden md:flex items-center space-x-10">
          {leftLinks.map((link) => (
            <Link
              key={link.name}
              to={link.href}
              className="relative group text-black-300 hover:text-black text-xs font-black uppercase tracking-widest transition-colors"
            >
              {link.name}
        ...
        {/* Centered Logo */}
        <div className="flex items-center group cursor-pointer relative" onClick={handleLogoClick}>
        ...
        <div className="hidden md:flex items-center space-x-10">
          {rightLinks.map((link) => (
            link.href.startsWith('#') ? (
              ...
              <Link key={link.name} to={link.href} className="relative group text-black-300 hover:text-black text-xs font-black uppercase tracking-widest transition-colors">
                {link.name}
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-[#005d30] transition-all group-hover:w-full"></span>
              </Link>
            )
        ...
        {/* Mobile Toggle */}
        <button onClick={() => setIsOpen(!isOpen)} className="md:hidden text-black p-2 z-101">
          <div className="w-6 h-4 relative flex flex-col justify-between">
            <span className={`h-0.5 bg-black transition-all ${isOpen ? 'rotate-45 translate-y-2 bg-white!' : 'w-6'}`}></span>
            <span className={`h-0.5 bg-black transition-all ${isOpen ? 'opacity-0' : 'w-4'}`}></span>
            <span className={`h-0.5 bg-black transition-all ${isOpen ? '-rotate-45 -translate-y-1.5 bg-white!' : 'w-5'}`}></span>
          </div>
        </button>
      </nav>

      {/* Mobile Menu */}
      <div className={`md:hidden fixed inset-0 bg-[#005d30] transition-all duration-700 ease-[cubic-bezier(0.85,0,0.15,1)] ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
